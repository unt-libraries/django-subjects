...
class AddSubject(forms.Form):
    ...
    keywords = forms.CharField(widget=Textarea(attrs={'cols': 70, 'rows': 5}),
                               label='', required=False)
    notes = forms.CharField(widget=Textarea(attrs={'cols': 70, 'rows': 5}),
                            label='', required=False)
...
def subject_add(request, sub_id=0, sub_name=''):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            s = Subject(name=clean_data['name'], keywords=clean_data['keywords'],
                        notes=clean_data['notes'], parent=sub_id, lft=1, rght=2)
            s.save()
