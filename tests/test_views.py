...
class TestSubjectViews:
	...
	def test_browse_subjects(self, client):
		...
		assert "<a href=\'/subjects/browse/{id}/\'>{name}</a>".format(id=s1.id, name=s1.name).encode() in response.content
		assert "<a href=\'/subjects/browse/{id}/\'>{name}</a>".format(id=s2.id, name=s2.name).encode() in response.content
	...
	def test_search_subjects(self, client):
		...
		assert '<span class="subject">{name}</span>'.format(name=subject.name).encode() in response.content
